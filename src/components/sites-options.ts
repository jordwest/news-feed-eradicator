...
export const SitesOptions = (store: Store) => {
    ...
    const stateText = (state: SiteStatus) => {
        switch (state.type) {
            ...
            case SiteStatusTag.ENABLED_TEMPORARILY:
                return h(
                    'div.pad-1.flex.align-items-center.justify-right.text-right.text-muted',
                    'Enabled (will disable in 10 mins to break your habit)'
                );
              
            case SiteStatusTag.ENABLED_FOR_15_MINUTES:
                return h(
                    'div.pad-1.flex.align-items-center.justify-right.text-right.text-muted',
    ...
    const Site = (id: SiteId, label: string) => {
        ...
        const onClick = () => {
            // Enable the site temporarily for 15 minutes
            store.dispatch({
